<script context='module'>
  import { BookUser, Type, Leaf, CalendarRange, MonitorPlay, MonitorUp, MonitorX, Tv, ArrowDownWideNarrow, Filter, FilterX, X, Tags, Hash, SlidersHorizontal, EyeOff, Hourglass, Mic, ImageUp, Search, Grid3X3, Grid2X2, BookOpen } from 'lucide-svelte'

  const badgeKeys = ['type', 'title', 'search', 'genre', 'genre_not', 'tag', 'tag_not', 'season', 'year', 'format', 'format_not', 'status', 'status_not', 'sort', 'hideSubs', 'hideMyAnime', 'hideStatus', 'showMyAnime', 'showStatus']
  const badgeDisplayNames = { type: BookOpen, title: BookUser, search: Type, genre: Hash, genre_not: Hash, tag: Hash, tag_not: Hash, season: CalendarRange, year: Leaf, format: Tv, format_not: MonitorUp, status: MonitorPlay, status_not: MonitorX, sort: ArrowDownWideNarrow, hideMyAnime: EyeOff, showMyAnime: Hourglass, hideSubs: Mic }
  ...
  const formatAnimeOptions = { TV: 'TV Show', MOVIE: 'Movie', TV_SHORT: 'TV Short', SPECIAL: 'Special', OVA: 'OVA', ONA: 'ONA' }
  const formatMangaOptions = { MANGA: 'Manga', NOVEL: 'Light Novel', ONE_SHOT: 'One Shot' }
...
<script>
  ...
  // Initialisation par défaut du type
  if (!search.type) search.type = 'ANIME'
  ...
  $: currentFormatOptions = search.type === 'MANGA' ? formatMangaOptions : formatAnimeOptions
  ...
  function toggleMediaType() {
    search.type = search.type === 'MANGA' ? 'ANIME' : 'MANGA'
    search.format = []
    search.format_not = []
    if (search.type === 'MANGA') {
      search.season = ''
      search.hideSubs = false
    }
    form.dispatchEvent(new Event('input', { bubbles: true }))
  }

  function searchClear() {
    const prevType = search.type || 'ANIME'
    search = {
      type: prevType,
  ...
  function getFormatDisplayName(value) {
    return currentFormatOptions[value] || formatAnimeOptions[value] || formatMangaOptions[value] || value
  }
...
<form class='container-fluid px-md-10 bg-dark position-sticky top-0 search-container z-40' class:px-10={search.fileEdit} class:px-md-50={!search.fileEdit} class:pt-20={!search.fileEdit} class:mt-20={!SUPPORTS.isAndroid && !search.fileEdit} class:mt-md-0={!SUPPORTS.isAndroid && !search.fileEdit} class:bg-very-dark={search.fileEdit} style:--container-color={search.fileEdit ? 'var(--dark-color-dim)' : 'var(--dark-color)'} on:input bind:this={form}>
  <div class='row'>
    ...
    <div class='col p-10 z-3 d-flex {advancedSearch} flex-column justify-content-end'>
      ...
      <div class='input-group'>
        <CustomDropdown id={`format-input`} bind:form options={currentFormatOptions} bind:value={search.format} bind:altValue={search.format_not} bind:disabled={search.disableSearch}/>
      </div>
    </div>
    ...
    {#if !search.fileEdit}
      <!-- Bouton Toggle Media Type (Anime / Manga) -->
      <div class='col-auto p-10 d-flex'>
        <div class='align-self-end'>
          <button class='btn btn-square bg-dark-light px-5 align-self-end border-0 z-1' type='button' data-toggle='tooltip' data-placement='bottom' data-target-breakpoint='md' data-title='{search.type === "MANGA" ? "Mode: Manga / Novel" : "Mode: Anime"}' use:click={toggleMediaType} disabled={search.disableSearch} class:text-primary={search.type === 'MANGA'}>
            <label for='media-type-toggle' class='pointer mb-0 d-flex align-items-center justify-content-center'>
              <BookOpen size='1.625rem' />
            </label>
          </button>
        </div>
      </div>
    ...
    <form>
      <div class='not-reactive' role='button' tabindex='0'>
        {#if sanitisedSearch?.length}
          {@const filteredBadges = sanitisedSearch.filter(badge => badge.key !== 'hideStatus' && badge.key !== 'showStatus' && (search.userList || badge.key !== 'title') && !(badge.key === 'type' && badge.value === 'ANIME'))}
          <div class='d-flex flex-wrap flex-row align-items-center'>
            ...
            {#each badgeKeys as key}
              ...
              {#each matchingBadges as badge}
                {#if badge.key === key && (badge.key !== 'hideStatus' && badge.key !== 'showStatus' && (search.userList || badge.key !== 'title')) && !(badge.key === 'sort' && badge.value === 'TRENDING_DESC') && !(badge.key === 'type' && badge.value === 'ANIME')}
                  <div use:click={() => removeBadge(badge)} class='badge border-0 py-5 px-10 text-capitalize mr-10 text-white text-nowrap d-flex align-items-center mb-5' style='max-width: 60vw' class:bg-dark-light={!badge.key.includes('_not')} class:bg-danger-very-dim={badge.key.includes('_not')}>
                    ...
                    <span class='text-truncate'>{badge.key === 'type' ? (badge.value === 'MANGA' ? 'Manga / Novel' : 'Anime') : badge.key === 'sort' ? getSortDisplayName(badge.value) : (badge.key === 'format' || badge.key === 'format_not') ? getFormatDisplayName(badge.value) : (badge.key === 'hideMyAnime' ? 'Hide My Anime' : badge.key === 'showMyAnime' ? 'Show My Anime' : badge.key === 'hideSubs' ? 'Dubbed' : ('' + badge.value).replace(/_/g, ' ').toLowerCase())}</span>
